...
const Cart = () => {
    ...
    const increaseQuantity = (id, quantity, stock) => {
        if (stock <= quantity) return console.log("The stock has run out");
        let newQty = quantity + 1;
        dispatch(addItemsToCart(id, newQty));
    };

    const decreaseQuantity = (id, quantity) => {
        if (1 >= quantity) return console.log("The stock has run out");
        let newQty = quantity - 1;
        dispatch(addItemsToCart(id, newQty));
    };
                      ...
                      <p>₹ 140</p>
                            ...
                            <div className="flex justify-between mt-3 w-[100%] text-3xl font-bold">
                                ...
                                <p> {cartItems &&
                                    cartItems.length > 0 &&
                                    `₹${cartItems.reduce(
                                        (acc, item) => (acc + item.quantity * item.price) + (acc + item.tax),
                                        0
                                    )}`}</p>
                            </div>
                        </div>
